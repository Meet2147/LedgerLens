...
const looksLikeDirectSupabaseConnection =
  typeof connectionString === "string" &&
  /\.supabase\.co(?::5432)?(?:[/?]|$)/i.test(connectionString) &&
...
function normalizeDatabaseError(error) {
  ...
  const address = String(error?.address || "");

  if (
    code === "ENETUNREACH" &&
    (address.includes(":") || looksLikeDirectSupabaseConnection)
  ) {
    return new Error(
      "Database connection failed because this deployment is trying to reach Supabase over an IPv6 direct connection. Render does not support Supabase direct IPv6 connections. In Supabase, open Connect and replace DATABASE_URL with the Supavisor session pooler string (pooler.supabase.com:5432), or enable the Supabase IPv4 add-on."
    );
  }
...
export async function getDatabaseStatus() {
  try {
    ...
    return {
      ...
      fallbackEnabled: isLocalFallbackEnabled(),
      error: normalizedError.message || "Unknown database error"
    };
  }
}
